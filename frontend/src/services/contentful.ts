...
export async function fetchRandomWords(count: number): Promise<WordEntry[]> {
  try {
    ...
    while (usedSkips.size < count && usedSkips.size < totalCount) {
      const randomSkip = Math.floor(Math.random() * totalCount)
      usedSkips.add(randomSkip)
    }
    
    // Create parallel API requests for all random skip values
    const fetchPromises = Array.from(usedSkips).map(async (skip) => {
      try {
        const response = await client.getEntries({
          content_type: 'englishDictation',
          limit: 1,
        ...
        console.warn(`Failed to fetch entry at skip ${skip}:`, entryError)
        return null
      }
    })
    ...
    // Filter out null results and return valid words
     const randomWords = results.filter((word): word is WordEntry => word !== null)
